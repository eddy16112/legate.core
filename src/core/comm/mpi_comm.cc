...
{
  log_coll.debug("Finalize MPINetwork %ld", mpi_comms.size());
                          ...
                          CollComm global_comm)
{
  ...
  CHECK_MPI(MPI_Alltoallv(sendbuf,
                          sendcounts,
                          sdispls,
                          mpi_type,
                          recvbuf,
...
#if 1
  MPI_Datatype mpi_type = dtypeToMPIDtype(type);
  CHECK_MPI(MPI_Allgather(sendbuf, count, mpi_type, recvbuf, count, mpi_type, MPI_COMM_WORLD));
#else
